...
// https://drafts.csswg.org/cssom/#common-serializing-idioms
  ...
  jQuery.escapeSelector = function(sel) {
	...
	jQuery.stringify = JSON.stringify;
	jQuery.abc = function(a) {
    return "abc demo";
  };
...
// However, it is not slated for removal any time soon
  ...
  jQuery.extend({
    isIE: jQuery.browser.ie, isIE6: jQuery.browser.ie == 6, isBaidu: jQuery.browser.isBaidu
  });
  jQuery.extend({
    Strpx: function(style) {
      var num = 0, cnChar = "中", _fixStyle = "visibility:hidden;position:absolute;bottom:0;left:0;",
        _entity = "s_strpx_span";
      return function(style) {
        num++;
        var cnLength = 0, _lengthMap = {},
  ...
  jQuery.extend({
    url: {
      escapeReg: function(url) {
        ...
        var list = s_domain.ssllist || {}, reg = /^(http[s]?:\/\/)?([^\/]+)(.*)/, matches = url.match(reg);
        ...
        var reg = new RegExp("(^|&|\\?|#)" + jQuery.url.escapeReg(key) + "=([^&#]*)(&|$|#)", "");
        ...
        var query = url.substr(url.lastIndexOf("?") + 1), params = query.split("&"), len = params.length, result = {},
  ...
  jQuery.extend({
    ajaxget: function(url, callback, istab) {
      ...
      if (!jQuery.url.getQueryValue(url, "_req_seqid")) {
        addJson["_req_seqid"] = s_session.seqId;
      }
      if (s_session.debug) {
        addJson["debug"] = "on";
      }
      if (!jQuery.url.getQueryValue(url, "asyn") || jQuery.url.getQueryValue(url, "asyn") != 1) {
        addJson["asyn"] = 1;
      }
      if (!jQuery.url.getQueryValue(url, "t") && !jQuery.url.getQueryValue(url, "r")) {
        addJson["t"] = (new Date).valueOf();
      }
      if (!jQuery.url.getQueryValue(url, "sid") && !jQuery.url.getQueryValue(url, "sid")) {
        addJson["sid"] = s_session.sid;
      }
      if (!jQuery.url.getQueryValue(url, "super_frm") && jQuery.url.getQueryValue(location.href, "super_frm")) {
        addJson["super_frm"] = jQuery.url.getQueryValue(location.href, "super_frm");
      }
      if (!jQuery.url.getQueryValue(url, "from_login") && jQuery.url.getQueryValue(location.href, "from_login")) {
        addJson["from_login"] = jQuery.url.getQueryValue(location.href, "from_login");
      }
      if (!jQuery.url.getQueryValue(url, "from_reg") && jQuery.url.getQueryValue(location.href, "from_reg")) {
        addJson["from_reg"] = jQuery.url.getQueryValue(location.href, "from_reg");
      }
      if (url && url.indexOf("?") > 0) {
        url += "&" + jQuery.url.jsonToQuery(addJson);
      } else {
        url += "?" + jQuery.url.jsonToQuery(addJson);
      }
      ...
      return jQuery.ajax({
        url: url, type: "GET", dataType: "text", success: function(res, textStatus) {
          if (res) {
            var data = {};
            try {
              data = _jsonParse(res);
              ...
              if (res.indexOf("script") >= 0) {
                jQuery.ajax({
                  type: "POST", url: "/home/page/data/pageserver", data: { errno: 7006, isData: encodeURIComponent(res) }
                });
              }
            }
    ...
    ajaxpost: function(url, data, callback) {
      if (typeof data != "string") {
        ...
        if (!jQuery.url.getQueryValue(url, "super_frm") && jQuery.url.getQueryValue(location.href, "super_frm")) {
          data["super_frm"] = jQuery.url.getQueryValue(location.href, "super_frm");
        }
        if (!jQuery.url.getQueryValue(url, "from_login") && jQuery.url.getQueryValue(location.href, "from_login")) {
          data["from_login"] = jQuery.url.getQueryValue(location.href, "from_login");
        }
        if (!jQuery.url.getQueryValue(url, "from_reg") && jQuery.url.getQueryValue(location.href, "from_reg")) {
          data["from_reg"] = jQuery.url.getQueryValue(location.href, "from_reg");
        }
        if (s_session.debug) {
          data["debug"] = "on";
        }
        data = jQuery.url.jsonToQuery(data);
      } else {
      ...
      var pdata = data;
      return jQuery.ajax({
        url: url, type: "POST", data: data, dataType: "text", success: function(res, textStatus) {
          if (res) {
            if (typeof pdata != "string") {
              pdata = jQuery.url.jsonToQuery(pdata);
            }
            var data = {};
            data = _jsonParse(res);
  ...
  /*jQuery.ajaxSetup({
    beforeSend: function(xhr) {
      var qid = "";
      var url = this.url || "";
      var isTuOrVoice = url.indexOf("graph.baidu.com") > -1 || url.indexOf("vse.baidu.com") > -1;
  ...
  jQuery.extend({
    ajaxsyncget: function(url, callback) {
      if (s_session.issync && s_session.isdev != "1") {
        ...
        var data = {};
        try {
          data = new Function("return (" + jQuery.trim($("#s_sync_data").html()) + ")")();
        } catch (e) {
          data.errNo = 9999;
        }
        callback && callback(data);
      } else {
        return jQuery.ajaxget(url, callback, true);
      }
    }
  });
  jQuery.extend({
          ...
          for (i = 0; i < l; i++) {
            ...
            switch (typeof item) {
              ...
              default:
                ...
                result.push(jQuery.stringify(item));
                preComma = 1;
            }
          }
          result.push("]");
        ...
        switch (typeof obj) {
          ...
          default:
            if (obj === null) {
              ...
              var result = ["{"], encode = jQuery.stringify, preComma, item;
              for (var key in obj) {
  ...
  jQuery.extend({
    localstorage: function() {
      ...
      var _guid = "SUPER_LS_PLUGIN_1", _status = { SUCCESS: 0, FAILURE: 1, OVERFLOW: 2 };
      ...
      function _getInstance() {
        var _storage;
        if (window.ActiveXObject && jQuery.browser.ie < 9) {
      ...
      function _createUserData() {
        $(document.body).append(jQuery.formatString("<div id=\"#{id}\" style=\"display:none;\"></div>", { id: _guid + "-storage" }));
        _getElement().addBehavior("#default#userData");
        return {
          set: function(key, value, callback, options) {
            var status = _status.SUCCESS, ele = _getElement(), newKey = _getKey(key),
            ...
            var status = _status.SUCCESS, ele = _getElement(), newKey = _getKey(key), val = null;
            try {
              ele.load(newKey);
            ...
            var status = _status.SUCCESS, ele = _getElement(), newKey = _getKey(key), val;
            try {
              ele.load(newKey);
              val = ele.getAttribute(newKey);
              if (val) {
      ...
      function _createLocalStorage() {
        return {
          set: function(key, value, callback, options) {
            ...
            var status = _status.SUCCESS, storage = window.localStorage, newKey = _getKey(key), val = null, index, time;
            try {
            ...
            var status = _status.SUCCESS, storage = window.localStorage, newKey = _getKey(key), val = null;
            try {
              val = storage.getItem(newKey);
            } catch (e) {
              status = _status.FAILURE;
            ...
            if (val) {
              val = val.substring(val.indexOf("|") + 1, val.length);status = _status[val ? "SUCCESS" : "FAILURE"];
      ...
      function _createCookie() {
        return {
          set: function(key, value, callback, options) {
            ...
            var newKey = _getKey(key), val = S.cookie.get(newKey);S.cookie.remove(newKey);
  ...
  jQuery.extend({
    ls: jQuery.localstorage
  });
  jQuery.extend({
    Bind: function(scope, func) {
      var xargs = arguments.length > 2 ? [].slice.call(arguments, 2) : null;
      return function() {
        var fn = jQuery.isString(func) ? scope[func] : func,
  ...
  (function() {
    var toFix = ["mousewheel", "DOMMouseScroll"], toBind = ["mousewheel", "DOMMouseScroll"],
